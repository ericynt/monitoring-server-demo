...
public class JWTTokenUtil {
    ...
    public static String createToken (User user, String tokenSecret) {
        String token = null;
        try {
            String[] roleStrings = Arrays.stream(user.getRoles())
                                         .map(User.Role::toString)
                                         .toArray(String[]::new);
            token = JWT.create()
                       .withSubject(user.getName())
                       .withExpiresAt(toDate(LocalDateTime.now()
                                                          .plusHours(HOURS)))
    ...
    public static void verifyJWTToken (String tokenSecret, User user, String token) throws
    ...
    public static String[] getRoleStrings (com.eric.monitoringserverjava.users.User retrievedUser) {
        return Arrays.stream(retrievedUser.getRoles())
                     .map(
