...
public class JobSyncer implements Runnable {
    ...
    @Autowired
    public JobSyncer (
            JobService jobService, RemoteService remoteService, RuleResultService ruleResultService,
    ...
    private void checkDBJob (Job j) {
        if (runningJobExecutors.stream()
                               .anyMatch(rj -> rj.getJob()
                                                 .getId()
                                                 .equals(j.getId()))) {
            return;
        }
    ...
    private void startJob (Job j) {
        JobExecutor jobExecutor = new JobExecutor(j, remoteService, ruleResultService, endpointService);
        jobExecutor.start();
